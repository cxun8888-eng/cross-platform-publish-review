...
| `debugger` 权限滥用或调试会话长期占用 | 安装时明确披露，只允许官方 Host 的受控命令，并在 `finally` detach |
...
基础存储权限为 `storage`，内容脚本只覆盖四个精确官方 Host。Chrome 不允许把 `debugger` 声明为可选权限，因此该能力在安装时明确披露，并保持：

1. 不在延迟流程中动态申请权限；
2. 只允许当前受支持的官方标签和可信消息类型；
3. 坐标有边界，closed Shadow DOM 按钮有精确文本/样式约束；
4. 每次操作后立即 detach；
5. 权限、连接或定位失败时给出分型提示，并可手动完成。
