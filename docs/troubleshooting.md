...
1. 确认扩展版本为 0.1.2 且处于启用状态；
2. 从 0.1.1 升级后，在扩展管理页重新加载或重新启用 PublishLoop，并刷新平台页面；
3. 只有明确提示“调试连接被占用”时，才关闭当前标签页的 DevTools 或暂停其他调试扩展；
4. 如果提示“没有找到唯一操作目标”，平台页面可能已经改版；
5. 使用平台原生“上传图片/图文”入口手动继续。

扩展不会自动选择本地文件。文件窗口打开后必须由用户操作；手动选图后，PublishLoop 仍会继续填充标题、正文和话题。
...
- 平台按钮不可用；
- 扩展升级后新增权限尚未确认；
- DevTools 或其他调试器占用调试连接；
- 平台 DOM 已更新。

PublishLoop 会区分权限异常、调试连接被占用和目标定位失败。使用平台原生按钮检查页面给出的错误；扩展不会绕过验证码、风控和二次确认。
